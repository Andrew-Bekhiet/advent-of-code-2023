import 'dart:developer';
import 'dart:io';
import 'dart:math' hide log;

import 'package:collection/collection.dart';

Future<void> main() async {
  const useExample = true;

  final Stopwatch parsing = Stopwatch()..start();
  final input = await parseInput(useExample);
  parsing.stop();

  final Stopwatch part1Stopwatch = Stopwatch()..start();
  int output1 = part1(input);
  part1Stopwatch.stop();

  final Stopwatch part2Stopwatch = Stopwatch()..start();
  int output2 = part2(input);
  part2Stopwatch.stop();

  printStats(parsing, part1Stopwatch, part2Stopwatch);

  printOutputs(output1, output2);

  checkAgainstExample(useExample, output1, output2);
}

Future<Almanac> parseInput(bool useExample) async {
  final inputFile =
      useExample ? File('Day-5/example-input.txt') : File('Day-5/input.txt');
  final String input = await inputFile.readAsString();

  return Almanac.parse(input);
}

int part1(Almanac almanac) {
  final combinedTransformers = combineTransformers([
    [
      LimitedOffsetTransformer(src: 0, dst: 0, length: 9223372036854775807),
    ],
    almanac.seedToSoilMap,
    almanac.soilToFertilizerMap,
    almanac.fertilizerToWaterMap,
    almanac.waterToLightMap,
    almanac.lightToTempMap,
    almanac.tempToHumidityMap,
    almanac.humidityToLocationMap,
  ]);

  return almanac.seeds
      .expand(
        (seed) =>
            combinedTransformers.map((t) => t.transform(seed)).whereNotNull(),
      )
      .min;
}

int traceToLocation(int seed, List<DynamicMap> maps) {
  if (maps.isEmpty) {
    log("$seed");
    log("end");
    return seed;
  }

  log("$seed =>");
  final int? newSeed = maps.firstOrNull?[seed];

  return traceToLocation(newSeed ?? seed, maps.sublist(1));
}

int part2(Almanac almanac) {
  final (realSeeds, _) = almanac.seeds.fold<(List<Interval>, int?)>(
    (<Interval>[], null),
    (acc, c) => (
      [...acc.$1, if (acc.$2 != null) Interval(acc.$2!, acc.$2! + c - 1)],
      acc.$2 != null ? null : c
    ),
  );

  final combinedTransformers = combineTransformers([
    almanac.seedToSoilMap,
    almanac.soilToFertilizerMap,
    almanac.fertilizerToWaterMap,
    almanac.waterToLightMap,
    almanac.lightToTempMap,
    almanac.tempToHumidityMap,
    almanac.humidityToLocationMap,
  ]);

  final LimitedOffsetTransformer largestNegativeShift =
      combinedTransformers.minOrNullBy((t) => t.offset)!;

  return largestNegativeShift.dstInterval.from;
}

List<LimitedOffsetTransformer> combineTransformers(
  List<List<LimitedOffsetTransformer>> transformers,
) {
  final [t1, t2, ..._] = transformers;

  final List<LimitedOffsetTransformer> combined = [...t1, ...t2]
      .sortedByCompare((t) => t.src, (p, n) => p.compareTo(n))
      .fold(
    [],
    (acc, c) => acc.isEmpty
        ? [c]
        : [...acc.sublist(0, acc.length - 1), ...acc.last.combineWith(c)],
  );
  /* t2.fold(
      t1,
      (acc, c) => acc
          .expand((a) => c.combineWith(a))
          .sortedByCompare((t) => t.src, (p, n) => p.compareTo(n))
          .toList()); */

  if (transformers.length < 3) return combined;

  return combineTransformers([combined, ...transformers.sublist(2)]);
}

void printStats(
    Stopwatch parsing, Stopwatch part1Stopwatch, Stopwatch part2Stopwatch) {
  print('Parsing took ${parsing.elapsedMilliseconds}ms');
  print('Part 1 took ${part1Stopwatch.elapsedMilliseconds}ms');
  print('Part 2 took ${part2Stopwatch.elapsedMilliseconds}ms');
}

void printOutputs(int output1, int output2) {
  log(output1.toString());
  print(output1);

  log(output2.toString());
  print(output2);
}

void checkAgainstExample(bool useExample, int output1, int output2) {
  if (!useExample) return;

  const expectedOutput1 = 35;
  const expectedOutput2 = 46;

  assert(output1 == expectedOutput1);
  assert(output2 == expectedOutput2);
}

class DynamicList {
  final int from;
  final int length;

  int get first => from;
  int get last => from + length - 1;

  DynamicList({required this.from, required this.length});

  int? operator [](int index) => index < length ? from + index : null;

  int? indexOf(int n) => n <= last ? n - from : null;

  bool contains(int n) => n >= first && n <= last;

  List<int> toList() => List.generate(length, (i) => this[i]!, growable: false);
}

class DynamicMap {
  final List<DynamicList> sources;
  final List<DynamicList> dests;

  DynamicMap({required this.sources, required this.dests})
      : assert(sources.length == dests.length),
        assert(sources.indexed.every((o) => dests[o.$1].length == o.$2.length));

  factory DynamicMap.parse(String input) {
    final (sources, dests) = input.trim().split('\n').map(
      (line) {
        final splittedLine = line.trim().split(' ');
        final [parsedDest, parsedSource, parsedLength] =
            splittedLine.map(int.parse).toList();

        return (
          DynamicList(from: parsedSource, length: parsedLength),
          DynamicList(from: parsedDest, length: parsedLength),
        );
      },
    ).fold(
      (<DynamicList>[], <DynamicList>[]),
      (acc, o) => ([...acc.$1, o.$1], [...acc.$2, o.$2]),
    );

    return DynamicMap(sources: sources, dests: dests);
  }

  int? operator [](int key) {
    final (int, DynamicList)? srcIndexInterval =
        sources.indexed.firstWhereOrNull((s) => s.$2.contains(key));

    return srcIndexInterval != null
        ? dests[srcIndexInterval.$1][srcIndexInterval.$2.indexOf(key)!]!
        : null;
  }
}

class Interval {
  final int from;
  final int to;

  Interval(this.from, this.to);

  bool contains(int n) => n >= from && n <= to;

  Interval? intersection(Interval other) {
    final int newFrom = max(from, other.from);
    final int newTo = min(to, other.to);

    if (newFrom > newTo) return null;

    return Interval(newFrom, newTo);
  }
}

class LimitedOffsetTransformer {
  final int src;
  final int dst;

  final int length;

  late final Interval srcInterval = Interval(src, src + length - 1);
  late final Interval dstInterval = Interval(dst, dst + length - 1);

  int get from => srcInterval.from;
  int get to => srcInterval.to;

  int get offset => dst - src;

  LimitedOffsetTransformer({
    required this.dst,
    required this.src,
    required this.length,
  });
  LimitedOffsetTransformer.fromIntervalOffset({
    required Interval interval,
    required int offset,
  })  : src = interval.from,
        dst = interval.from + offset,
        length = interval.to - interval.from + 1;

  factory LimitedOffsetTransformer.parse(String line) {
    final splittedLine = line.trim().split(' ');
    final [parsedDst, parsedSrc, parsedLength] =
        splittedLine.map(int.parse).toList();

    return LimitedOffsetTransformer(
      src: parsedSrc,
      dst: parsedDst,
      length: parsedLength,
    );
  }

  bool contains(int n) => srcInterval.contains(n);

  int? transform(int n) => contains(n) ? n + offset : null;

  List<LimitedOffsetTransformer> combineWith(LimitedOffsetTransformer other) {
    final sortedTransformers =
        [this, other].sortedByCompare((t) => t.src, (p, n) => p.compareTo(n));

    final Interval? intersection = dstInterval.intersection(other.srcInterval);

    if (intersection == null) return sortedTransformers;

    final firstTransformer =
        min(dstInterval.from, other.srcInterval.from) == dstInterval.from
            ? this
            : other;

    final firstInterval =
        firstTransformer == this ? dstInterval : other.srcInterval;

    final secondTransformer =
        max(dstInterval.to, other.srcInterval.to) == dstInterval.to
            ? this
            : other;

    final secondInterval =
        secondTransformer == this ? dstInterval : other.srcInterval;

    final Interval before = Interval(
      firstInterval.from,
      intersection.from - 1,
    );
    final Interval after = Interval(
      intersection.to + 1,
      secondInterval.to,
    );

    return [
      LimitedOffsetTransformer.fromIntervalOffset(
        interval: before,
        offset: firstTransformer.offset,
      ),
      LimitedOffsetTransformer.fromIntervalOffset(
        interval: intersection,
        offset: this.offset + other.offset,
      ),
      LimitedOffsetTransformer.fromIntervalOffset(
        interval: after,
        offset: secondTransformer.offset,
      ),
    ].where((t) => t.srcInterval.from < t.srcInterval.to).toList();
  }
}

class Almanac {
  final List<int> seeds;
  final List<LimitedOffsetTransformer> seedToSoilMap;
  final List<LimitedOffsetTransformer> soilToFertilizerMap;
  final List<LimitedOffsetTransformer> fertilizerToWaterMap;
  final List<LimitedOffsetTransformer> waterToLightMap;
  final List<LimitedOffsetTransformer> lightToTempMap;
  final List<LimitedOffsetTransformer> tempToHumidityMap;
  final List<LimitedOffsetTransformer> humidityToLocationMap;

  Almanac({
    required this.seeds,
    required this.seedToSoilMap,
    required this.soilToFertilizerMap,
    required this.fertilizerToWaterMap,
    required this.waterToLightMap,
    required this.lightToTempMap,
    required this.tempToHumidityMap,
    required this.humidityToLocationMap,
  });

  factory Almanac.parse(String input) {
    final List<String> sections = input.split('\n\n');

    final [_, seeds] = sections[0].split(':');
    final List<int> parsedSeeds =
        seeds.trim().split(' ').map(int.parse).toList();

    final List<LimitedOffsetTransformer> seedToSoilMap =
        parseTransformers(sections[1]);
    final List<LimitedOffsetTransformer> soilToFertilizerMap =
        parseTransformers(sections[2]);
    final List<LimitedOffsetTransformer> fertilizerToWaterMap =
        parseTransformers(sections[3]);
    final List<LimitedOffsetTransformer> waterToLightMap =
        parseTransformers(sections[4]);
    final List<LimitedOffsetTransformer> lightToTempMap =
        parseTransformers(sections[5]);
    final List<LimitedOffsetTransformer> tempToHumidityMap =
        parseTransformers(sections[6]);
    final List<LimitedOffsetTransformer> humidityToLocationMap =
        parseTransformers(sections[7]);

    return Almanac(
      seeds: parsedSeeds,
      seedToSoilMap: seedToSoilMap,
      soilToFertilizerMap: soilToFertilizerMap,
      fertilizerToWaterMap: fertilizerToWaterMap,
      waterToLightMap: waterToLightMap,
      lightToTempMap: lightToTempMap,
      tempToHumidityMap: tempToHumidityMap,
      humidityToLocationMap: humidityToLocationMap,
    );
  }
}

List<LimitedOffsetTransformer> parseTransformers(String section) {
  final [_, ...content] = section.trim().split('\n');

  final List<LimitedOffsetTransformer> transformers =
      content.map(LimitedOffsetTransformer.parse).toList();

  final List<Interval> intervals =
      transformers.map((t) => t.srcInterval).toList();

  return transformers;
}

extension on List<Interval> {
  bool hasInt(int n) => any((i) => i.contains(n));
}

extension MinBy<T> on Iterable<T> {
  T? minOrNullBy(int Function(T) selector) {
    var iterator = this.iterator;
    if (iterator.moveNext()) {
      var value = selector(iterator.current);
      while (iterator.moveNext()) {
        var newValue = selector(iterator.current);
        if (newValue < value) {
          value = newValue;
        }
      }
      return iterator.current;
    }
    return null;
  }
}
